...
- (void)logoBtnTouchUpInside:(UIButton *)btn {
    [self resumeAllPopViewAndBtnWithout:btn];
...
- (void)emojiBtnSelect:(UIButton *)btn {
    ...
    btn.selected = !btn.selected;
    [self updateArrow:btn];
    [self updateEmojiBtnsHiddenWithEmojiBtn:btn];
...
- (void)resumeAllPopViewAndBtnWithout:(UIButton *)exceptBtn {
    BOOL exceptBtnOldStatus = exceptBtn.selected;
    ...
    //把例外的按钮选择状态设置回来
    if (exceptBtn && exceptBtnOldStatus) {
        exceptBtn.selected = exceptBtnOldStatus;
    }
    //删除所有popView
    [self.responderKBViewController.rootWrapView removeAllOtherPopView:nil];
}
...
- (void)updateEmojiBtnsHiddenWithEmojiBtn:(UIButton *)emojiBtn {
    _logoBtn.hidden = _switchkbBtn.hidden = _skinBtn.hidden = emojiBtn.selected;
    _phraseBtn.hidden = _emoticonBtn.hidden = _graphicBtn.hidden = !emojiBtn.selected;
}
